...
class IdeaPage extends Component {
    ...
    deleteIdea = (ideaId) => {
        fetch(`/api/users/${this.props.match.params.userId}/ideas/${ideaId}`, { method: 'delete' })
        .then((res) => {
            return res.json();
        }).then((json) => {
            this.setState({user: json.data});
        ...
        return (
            <div>
                <div style={{ margin: 'auto', padding: '15px' }}>
                    ...
                    <button onClick={this.createIdea} style={{ fontSize: '20px', margin: '5px' }}>New Idea</button>
                </div>
                {this.state.user &&
                    <IdeaView user={this.state.user} deleteIdea={this.deleteIdea}
